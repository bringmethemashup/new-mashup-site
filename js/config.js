...
/**
 * Per-mashup share pages, base URL (trailing slash required). Sharing a mashup
 * hands out `<SHARE_URL><id>.html` - a tiny static page on GitHub Pages that
 * carries that mashup's Open Graph tags (title + generated artist/waveform
 * card) for crawlers and bounces real visitors into the app at #track=<id>.
 * Pages are generated by scripts/build-share-pages.mjs.
 *
 * These used to come from the Supabase edge function `share`, but Supabase now
 * forces content-type text/plain on HTML from edge functions, so that page
 * stopped redirecting and friends just saw its source. The card IMAGE is still
 * rendered by that function (?t=<id>&img=1) - PNG responses are unaffected.
 */
export const SHARE_URL = 'https://bringmethemashup.github.io/new-mashup-site/s/';
